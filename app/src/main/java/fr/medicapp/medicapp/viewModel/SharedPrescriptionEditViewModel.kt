...
class SharedPrescriptionEditViewModel(
    ...
    fun updatePosology(newPosology: String) {
        val updatedPrescriptionInformation = _sharedState.value.prescriptionInformation.copy(posology = newPosology)
        val updatedPrescription = _sharedState.value.copy(prescriptionInformation = updatedPrescriptionInformation)
        _sharedState.value = updatedPrescription
    }

    fun updateFrequency(newFrequency: String) {
        val updatedPrescriptionInformation = _sharedState.value.prescriptionInformation.copy(frequency = newFrequency)
        val updatedPrescription = _sharedState.value.copy(prescriptionInformation = updatedPrescriptionInformation)
        _sharedState.value = updatedPrescription
    }
    ...
    fun updateNotificationActiveState(index: Int, newActiveState: Boolean) {
        val updatedNotifications = _sharedState.value.notifications.toMutableList()
        val updatedNotificationInformation = updatedNotifications[index].notificationInformation!!.copy(active = newActiveState)
        updatedNotifications[index] = updatedNotifications[index].copy(notificationInformation = updatedNotificationInformation)
        val updatedPrescription = _sharedState.value.copy(notifications = updatedNotifications)
        _sharedState.value = updatedPrescription
    }
    ...
    fun updateNotificationDays(index: Int, dayOfWeek: DayOfWeek) {
        ...
        val updatedNotificationInformation = notificationInformationToUpdate.copy(days = notificationInformationToUpdate.days)
        updatedNotifications[index] = updatedNotifications[index].copy(notificationInformation = updatedNotificationInformation)
        val updatedPrescription = _sharedState.value.copy(notifications = updatedNotifications)
        _sharedState.value = updatedPrescription
    }
    ...
    fun save(context: Context) {
        ...
        PrescriptionRepository(context).insert(_sharedState.value)
    }
