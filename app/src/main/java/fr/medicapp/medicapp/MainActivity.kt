    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Thread {
            ...
            val medication: MedicationEntity =
                gson.fromJson(monoprost, Medication::class.java).convert(this)

            Log.d("ObjectBox", "MedicationEntity : $medication")

            // si le cisCode n'existe pas dans la base de données, on l'ajoute
            if (store.query().equal(MedicationEntity_.cisCode, medication.cisCode).build().find()
                    .isEmpty()
            ) {
                Log.d("ObjectBox", "Ajout de Monoprost")
                store.put(medication)
                val medicationTmp =
                    store.query().equal(MedicationEntity_.cisCode, medication.cisCode).build()
                        .find()[0]
